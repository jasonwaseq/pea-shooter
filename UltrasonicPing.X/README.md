...
| `GND` | Board `GND` |
| `TRIG` | `PortX-10` / `RD7` |
| `ECHO` | `PortY-11` / `RE5`, through the divider below |
...
```text
HC-SR04 ECHO --- 1.2 kOhm ---+--- PortY-11 / RE5
                             |
                           2.2 kOhm
                             |
                            GND
...
`TRIG` on `PortX-10`, `ECHO` on `PortY-11`.
...
- Measures the returning `ECHO` pulse on `PortY-11`.
